...
This project is a software replacement for AnywhereUSB, which is a commercial product designed for USB over IP sharing. It explicity does not provide the  security features of the commercial product. This is to make it easy to deploy and operate in a controlled network environment where security can be managed at the network level.
